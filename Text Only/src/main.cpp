    ...
    if(millis() > time_elapsed)  // update every one second
    {
      if(leadoff_deteted == true) // lead in not connected
      {
        // Serial.println("ECG lead error!!! ensure the leads are properly connected");
          // Serial.println(-1);
          for(int i=0; i<4; i++)
            HeartRateBuffer[i] = 0xff;
      }
      else
      {
        HeartRateBuffer[0]=0x01;
        HeartRateBuffer[1]=0x01;
        HeartRateBuffer[2]=(uint8_t)global_HeartRate;
        HeartRateBuffer[3]={};
        for(int i=0;i<3;i++)
        {
          HeartRateBuffer[3]+=HeartRateBuffer[i];
        }
        for(int i=0;i<4;i++)
          Serial.print(HeartRateBuffer[i]);
        // Serial.print("Heart rate: ");
        // Serial.println(global_HeartRate,HEX);
        // Serial.println("BPM");
      }
      time_elapsed += 1000;
    }
